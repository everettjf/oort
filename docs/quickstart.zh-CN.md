...
oort 客户机基于 Ubuntu 24.04 ARM64 云镜像：
...
1. 用 Go 交叉编译客户机 agent（`oort-guest`，linux/arm64）到共享目录 `share/`；
...
`oort start` 会一直等到 Docker 就绪，然后打印 `DOCKER_HOST`。

> 首次配置通常几十秒（取决于 Docker CDN 速度）。之后复用同一块盘启动只需几秒。

## 5. 使用 Docker

让原生 `docker` CLI 指向 oort 的守护进程：
...
```bash
oort env        # 输出： export DOCKER_HOST=unix://...
eval "$(oort env)"
```
...
oort docker run --rm -v /mnt/mac:/m alpine cat /m/note.txt   # 输出 hello
```
...
```bash
oort docker run --rm --platform linux/amd64 alpine uname -m   # 输出 x86_64
```
...
oort shell                # 简易交互式 shell（逐行）
```

## 10. 查看状态 / 停止

```bash
oort status               # VM 与 Docker 状态
