...
oort 的目标是用最少的自研代码，复刻 OrbStack「Docker 跑在轻量 Linux VM 里、却像本地一样无缝」的核心体验。本文讲清楚每一层是怎么实现的。
...
```
   ...
   │  oort（Swift 主程序）                    │◀──────▶│    vsock 2377 → tcp 端口转发           │
...
`Sources/oort/VMConfig.swift` 用 VZ 构建一台 VM：
...
VZ 直接走 Apple Silicon 的硬件虚拟化，CPU 几乎零损耗，启动 1–2 秒。运行时需要 `com.apple.security.virtualization` entitlement，所以二进制要 codesign（`run.sh` / `oort` 自动处理）。
...
- **客户机侧 `oort-guest`**：vsock 2375 上 accept，dial `/run/docker.sock`，splice。
...
| 2376 | exec：读 HTTP body 当 shell 命令执行，回传输出（`oort exec` 的后端） |
...
- 客户机 agent：从共享盘安装编译好的 `oort-guest`；
