...
VZ 直接走 Apple Silicon 的硬件虚拟化，CPU 几乎零损耗，启动 1–2 秒。运行时需要 `com.apple.security.virtualization` entitlement，所以二进制要 codesign（`run.sh` / `oorb` 自动处理）。
...
| 2376 | exec：读 HTTP body 当 shell 命令执行，回传输出（`oorb exec` 的后端） |
