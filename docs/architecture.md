...
oort aims to reproduce OrbStack's core experience — "Docker runs in a lightweight Linux VM,
...
```
   ...
   │  oort (Swift main program)               │◀──────▶│    vsock 2377 → tcp port-forward      │
...
`Sources/oort/VMConfig.swift` builds a VM with VZ:
...
codesigned (`run.sh` / `oort` do this automatically).
...
```
docker CLI → ~/.oort/docker.sock  →  [vsock 2375]  →  oort-guest  →  /run/docker.sock → dockerd
...
- **Guest side `oort-guest`**: accepts on vsock 2375, dials `/run/docker.sock`, splices.
...
| 2376 | exec: read an HTTP body as a shell command, run it, return output (backend for `oort exec`) |
...
- Guest agent: installs the compiled `oort-guest` from the share;
