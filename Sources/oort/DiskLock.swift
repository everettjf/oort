...
/// `oort` wrapper has a best-effort `pgrep` guard, but that's racy (TOCTOU) and
...
final class DiskLock {
    ...
    /// Acquire the lock, or throw if another oort VM already holds it.
        ...
        if flock(fd, LOCK_EX | LOCK_NB) != 0 {
            ...
            if e == EWOULDBLOCK {
                throw CLIError.runtime(
                    "disk '\(diskImage.lastPathComponent)' is already in use by another oort VM — "
                    + "run 'oort stop' first (concurrent writers would corrupt it)")
            }
