...
guest:
	cd guest-agent && GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -ldflags="-s -w" -o ../share/oort-guest .

# Static checks that don't need a VM (CI-friendly): builds + shellcheck + go vet.
lint:
	swift build -c release >/dev/null
	cd guest-agent && go vet ./...
	@command -v shellcheck >/dev/null && shellcheck -S warning oort tests/e2e.sh make-image.sh || echo "shellcheck not installed; skipping"
