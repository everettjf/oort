#!/usr/bin/env bash
# bench.sh — a small performance baseline for oort.
#
...
#   - Docker run latency
#
# Filesystem tests run directly in the guest via `oort exec` (Ubuntu's GNU
# coreutils → accurate rates and nanosecond timing, no container-startup noise).
# Requires the VM to be started:  ./oort start && ./bench.sh
set -uo pipefail
...
echo
echo "════════════════════════════════════════════════════════"
echo " oort benchmark  (size=${SIZE_MB}MB, small-files=${SMALL_N})"
